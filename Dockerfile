...
# Install phantomjs only for ARM64
RUN \
...
# Use architecture-specific base image
# FROM ghcr.io/linuxserver/baseimage-kasmvnc:debianbookworm-876361b9-ls121 AS buildstage-amd64
# FROM ghcr.io/linuxserver/baseimage-kasmvnc:arm64v8-debianbookworm-876361b9-ls121 AS buildstage-arm64

# Select the appropriate base image based on target architecture
# FROM buildstage-${TARGETARCH} AS buildstage
...
# Sparrow - architecture-specific installation
RUN \
    ...
    wget --quiet https://github.com/sparrowwallet/sparrow/releases/download/${SPARROW_VERSION}/sparrowwallet_${SPARROW_DEBVERSION}_${SPARROW_ARCH}.deb \
    ...
    DEBIAN_FRONTEND=noninteractive \
    apt-get install -y ./sparrowwallet_${SPARROW_DEBVERSION}_${SPARROW_ARCH}.deb && \
    # cleanup
